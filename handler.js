...
module.exports = index = async(conn, m, warn, store) =>{
try {
...
return buffer
	...
	const fake = {
	 ...
	 { remoteJid: "6283898869011-1589321480@g.us" } : {}) 
                },
	 message: { 
		"extendedTextMessage": {
	...
	const fdoc = {key : {participant : '0@s.whatsapp.net'},message: {documentMessage: {title: `*Iron - Bot*`,jpegThumbnail: global.thumb}}}
	const fgclink = {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "6283898869011-1589321480@g.us","inviteCode": "m","groupName": "Iron Bot", "caption": `© Shwva_Tyyy`, 'jpegThumbnail': global.thumb}}}
    const fvideo = {key: { fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283898869011-1589321480@g.us" } : {}) },message: { "videoMessage": { "title":`*AUTO DOWNLOAD AUDIO YOUTUBE*`, "h": `Hmm`,'seconds': '10000000⁰0', 'caption': `*AUTO DOWNLOAD AUDIO YOUTUBE*`, 'jpegThumbnail': global.thumb}}}
...
switch(command){
			...
			case'help': {
				...
				anu = `${fke}Hi ${m.pushName} 👋, i'am Iron!${fke}
...
`,
					btn = [{
                                urlButton: {
                                    ...
                                    url: 'https://instagram.com/shwva_tyyy'
                                }
                            }, {
                                urlButton: {
				...
				conn.sendMessage(from, {text : `*This is an ad*\n\n•───────────•\n${jsn[0].setting}\n•───────────•\n\n_© 2022 shwvatyyy.net_`}, {quoted :{
				key: { fromMe: false,
				participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283898869011-1589321480@g.us" } : {})},
...
Konfirmasi pembayaran mu dengan menunjjukan bukti transfer kepada owner!
			...
			case 'qriscrizzy':{
				...
				bta = [
						...
						conn.sendMessage(from, { caption: captionQris, image: { url: 'https://topindo-warehouse.id:8081/ID2022163113071_A01.png' } , buttons: bta, footer: `Thank you for your help and support to this bot, hopefully the sustenance will be smoother. \n\n© 2022 Shwva_Tyyy` })
						} 
            ...
            case 'id': {
             m.reply(from)
              ...
              break
			case 'join': {
                if (!isPremium & !isCreator) return m.reply('premium only')
			...
			case 'tk': case 'tarikkomisi': {
				...
				let nyobb = list
				conn.sendMessage(from, {text:`*PILLIH E-WALLET*\nPilih jenis wallet yang ingin anda gunakan!`, title: 'WALLET', footer: '© 2022 Shwva_Tyyy', buttons: nyobb})
				}
				break
			...
			case 'ytplay': {
                ...
                let buttons = [
                	{ urlButton: { displayText: `𝐒𝐮𝐩𝐩𝐨𝐫𝐭 𝐔𝐬!`, url : `https://instagram.com/shwva_tyyy` } },
...
⊳ Url : ${anu.url}`,
			...
			case 'dariytskunci':{ //KUNCI = BIAR GA DR AKSES HEHE
                ...
                conn.sendMessage(from, { caption: `⭔ Title : ${resmed.title}\n⭔ File Size : ${resmed.filesizeF}\n⭔ Url : ${isUrl(text)}\n⭔ Ext : MP3\n⭔ Resolusi : ${args[1] || '128kbps'}`, image: {url: resmed.thumb}, templateButtons: btnYts, footer: `© 2022 shwvatyyy.net` })
				}
				break
			...
			case 'tt':
				...
				let nih = ` *TIKTOK DOWNLOADER BY IRON-BOT*

				©️ *Creator:* Shwva_Tyyy
				📌 *Title:* ${p.title}
				👤 *Author:* ${p.author}
...
break*/
			...
			let buttons = [
                ...
                conn.sendMessage(from, {text:`*NGETES BUTTON*`, title: 'WALLET', footer: '© 2022 Shwva_Tyyy', buttons: buttons})
                }
                break
			case 'request': case 'requestfitur':{
				if (!text) return m.reply(`Text mana?\n\nExample : ${prefix + command} Tambahin fitur tiktok downloader dong`)
				conn.sendMessage(`6283898869011@s.whatsapp.net`, {text: `*Hallo owner ada yang request fitur nih*\n\nPesan : ${text}\nPengirim : ${m.sender}`}, {quoted: {key: { fromMe: false,
				participant: `${m.sender}`, ...(from ? { remoteJid: "6283898869011-1589321480@g.us" } : {})},
				message: { "extendedTextMessage": {
				"text": `Request Fitur!`,
				"title": `Pasang iklan murah bang`,
				'jpegThumbnail': global.thumb}}}})
...
@${orang.split('@')[0]} ❤️ @${jodoh.split('@')[0]}`
            ...
            case 'repeat': {
                let text1 = q.split("|")[0]
                let text2 = q.split("|")[1]
                  if (!text1) return m.reply(`Contoh! : ${command + ' ' + 'tes|30'}`)    
                  if (!text2) return m.reply(`Contoh! : ${command + ' ' + 'tes|30'}`)
           ...
           break
			...
			case 'copystyle':
  			  ...
  			  await conn.sendMessage(from, { text: `${captionnye}`, templateButtons: menuButa, footer: `Terimakasih telah menggunan Iron-Bot\nDonasi untuk suport bot!\n\n© 2022 Shwva_Tyyy` })
				break
	  	  case 'style': case 'styletext': {
				...
				var listms = { text: `*Srtle Text*\n\nBerhasil menemukan sytle font\nSilahkan pilih font yang anda suka.`, footer: `Terimakasih telah menggunan Iron-Bot\nDonasi untuk suport bot!\n\n© 2022 Shwva_Tyyy`, buttonText: "Click Here", sections }
			...
			case 'anticall': {
                ...
                m.reply(`AntiCall Tidak Aktif !`)
                 ...
                 let buttons = [
                        { buttonId: 'anticall enable', buttonText: { displayText: 'Enable' }, type: 1 },
                        { buttonId: 'anticall disable', buttonText: { displayText: 'Disable' }, type: 1 }
                    ]
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
	        ...
	        case 'attp': case 'ttp': {
         	   ...
         	   await conn.sendMedia(from, `https://xteam.xyz/${command}?file&text=${text}`, 'Shwva', 'Tyyy', m, {asSticker: true})
    		    }
     		   break
           ...
           case 'setppbot': {
	        ...
	        case 'tourl': {
          	  ...
          	  break
			case 'name':
				conn.sendMessage(from, {text: conn.getName('6283898869011@s.whatsapp.net')})
				break
            case 'sound1':
            case 'sound2':
            case 'sound3':
...
await conn.sendMessage(m.chat, { audio: zens_dev, mimetype: 'audio/mp4', ptt: true }, { quoted: fdoc })     
break
//++++++++++++++ END CASE +++++++++++++++
default:
...
if ((m.mtype === 'reactionMessage') && m.isGroup) {
    conn.sendMessage(from, {text: `Terdeteksi *${m.pushName} @${m.sender.split("@")[0]}* Mengirim Reaction.`, mentions: [m.sender]}, {quoted: fake})
        ...
        if ((m.mtype === 'groupInviteMessage' || budy.startsWith('https://chat') || budy.startsWith('Buka tautan ini')) && !m.isBaileys && !m.isGroup) {
			...
			conn.sendMessage(from, {text:`INVITE DETECT\n\nMau masukin bot ini ke group kamu?`, title: 'LINK GROUP DECTETED', footer: '© 2022 Shwva_Tyyy', buttons: buttons, headerType: 5})
		}
...
print(e)
conn.sendMessage(`6283898869011@s.whatsapp.net`, { text: String(e) }, { quoted: m})
}
}
}
} catch(e) {
